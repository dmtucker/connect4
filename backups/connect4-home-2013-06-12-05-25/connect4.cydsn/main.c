...
uint8 get_move () {
    ...
    do {
        ...
        CapSense_UpdateEnabledBaselines(); //Helps avoid auto-dropping?
        CapSense_ScanEnabledWidgets();
        while (CapSense_IsBusy());
        if ((player1_turn && CapSense_CheckIsWidgetActive(CapSense_BUTTON0__BTN))
        || (!player1_turn && CapSense_CheckIsWidgetActive(CapSense_BUTTON1__BTN))) { //Helps avoid auto-dropping?
...
void receive_packet ( play_t * move ) {
    ...
    while (true) {
        ...
        LCD_Position(1,9);
        LCD_PrintNumber(UART_GetRxBufferSize());
        CyDelay(1000);
        /**/
